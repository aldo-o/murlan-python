export default class PreloadScene extends Phaser.Scene {
    ...
    create() {
        var bg = Utils.SpriteSettingsControl(this, Math.round(this.game.config.width / 2), Math.round(this.game.config.height / 2), "splash_bg", 0.5, 0.5, scaleFactor, scaleFactor);

        var logo = Utils.SpriteSettingsControl(this, Math.round(this.game.config.width / 2), Math.round(this.game.config.height / 3.7), "logo", 0.5, 0.5, scaleFactor, scaleFactor);

        this.progressBase = Utils.SpriteSettingsControl(this, Math.round(this.game.config.width / 2), Math.round(this.game.config.height / 1.25), "progress_base", 0.5, 0.5, scaleFactor, scaleFactor);

        this.progressBar = Utils.SpriteSettingsControl(this, Math.round(this.game.config.width / 2), Math.round(this.game.config.height / 1.25), "progress_bar", 0.5, 0.5, scaleFactor, scaleFactor);
        this.progressBar.setCrop(0, 0, 0, this.progressBar.height);
        this.loadingText = this.add.text(Math.round(this.game.config.width / 2), Math.round(this.game.config.height / 1.2), "Loading: ", { fontFamily: 'Poppins_Bold', fontSize: '46px', fill: '#FFF', fontStyle: "normal", align: 'center' }).setOrigin(0.5, 0.5).setScale(scaleFactor, scaleFactor);
